...
//       ...selectedLanguage,
//       name: lang,
//       language: lang === "en" ? "En" : "Br",
//     });
//     setRtl(false); // Assuming neither "en" nor "br" require RTL layout
//     if (pathname) {
//       router.push(`/${lang}/${pathname.split("/")[2]}`);
//     }
//   };
...
const Language = () => {
  ...
  const handleSelected = (lang: string) => {
    setSelectedLanguage({
      ...selectedLanguage,
      name: lang,
      language: lang === "br" ? "Br" : "Br",
    });
    setRtl(false); // Assuming "br" does not require RTL layout
    if (pathname) {
      router.push(`/${lang}/${pathname.split("/")[2]}`);
    }
  };
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button type="button" className="bg-transparent hover:bg-transparent">
          <span className="w-6 h-6 rounded-full me-1.5">
            <Image
              src={selectedLanguage ? selectedLanguage.flag : flag4}
              alt=""
              className="w-full h-full object-cover rounded-full"
            />
          </span>
          <span className="text-sm text-default-600 capitalize">
            {selectedLanguage ? selectedLanguage.name : "Br"}
          </span>
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent className="p-2">
